...
class RenameApp:
    ...
    def _build_ui(self) -> None:
        ...
        style.configure("Card.TLabelframe", background="#FFFFFF", bordercolor="#DADDE3", borderwidth=1)
        style.configure("Card.TLabelframe.Label", background="#FFFFFF", foreground="#202124", font=("Microsoft YaHei UI", 10, "bold"))

        title_frame = tk.Frame(self.root, bg="#FFFFFF", padx=16, pady=12)
        title_frame.pack(fill=tk.X, padx=10, pady=(10, 6))
        tk.Label(title_frame, text="D3_TOOL 批量拼音重命名工具", bg="#FFFFFF", fg="#111827", font=("Microsoft YaHei UI", 16, "bold")).pack(side=tk.LEFT)
        tk.Label(title_frame, text="支持文件 / 文件夹中文转拼音、日期后缀、版本号、预览导出", bg="#FFFFFF", fg="#6B7280", font=("Microsoft YaHei UI", 10)).pack(side=tk.LEFT, padx=16)
        tk.Label(title_frame, text="By: 印象视界_程阳", bg="#FFFFFF", fg="#111827", font=("Microsoft YaHei UI", 10, "bold")).pack(side=tk.RIGHT)

        body = tk.Frame(self.root, bg="#F4F5F7")
        body.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))

        step1 = ttk.LabelFrame(body, text="① 选择对象", style="Card.TLabelframe", padding=10)
        step1.pack(fill=tk.X, pady=(0, 8))
        ttk.Button(step1, text="选择文件夹", command=self.choose_folder).pack(side=tk.LEFT)
        ttk.Button(step1, text="选择文件", command=self.choose_files).pack(side=tk.LEFT, padx=(8, 16))
        self.mode_label = tk.Label(step1, textvariable=self.mode_text, bg="#FFFFFF", fg="#DC2626", font=("Microsoft YaHei UI", 10, "bold"))
        self.mode_label.pack(side=tk.LEFT)
        tk.Label(step1, textvariable=self.selected_path, bg="#FFFFFF", fg="#6B7280", font=("Microsoft YaHei UI", 10)).pack(side=tk.LEFT, padx=(14, 0))

        step2 = ttk.LabelFrame(body, text="② 设置命名规则", style="Card.TLabelframe", padding=10)
        step2.pack(fill=tk.X, pady=(0, 8))
        tk.Label(step2, text="拼音模式：", bg="#FFFFFF").grid(row=0, column=0, sticky="w")
        self.pinyin_cb = ttk.Combobox(step2, state="readonly", values=list(self.PINYIN_LABEL_TO_KEY.keys()), width=32)
        self.pinyin_cb.grid(row=0, column=1, padx=6, sticky="w")
        self.pinyin_cb.set(self.PINYIN_MODE_LABELS["full_title"])
        self.pinyin_cb.bind("<<ComboboxSelected>>", self._on_rule_change)

        tk.Label(step2, text="分隔符：", bg="#FFFFFF").grid(row=0, column=2, padx=(16, 0), sticky="w")
        self.sep_cb = ttk.Combobox(step2, state="readonly", values=list(self.SEP_LABEL_TO_KEY.keys()), width=12)
        self.sep_cb.grid(row=0, column=3, padx=6, sticky="w")
        self.sep_cb.set(self.SEP_LABELS[""])
        self.sep_cb.bind("<<ComboboxSelected>>", self._on_rule_change)

        tk.Label(step2, text="示例：", bg="#FFFFFF", fg="#6B7280").grid(row=1, column=0, pady=(8, 0), sticky="w")
        tk.Label(step2, textvariable=self.rule_preview_var, bg="#FFFFFF", fg="#2563EB", font=("Microsoft YaHei UI", 10, "bold")).grid(row=1, column=1, columnspan=4, pady=(8, 0), sticky="w")

        step3 = ttk.LabelFrame(body, text="③ 后缀选项", style="Card.TLabelframe", padding=10)
        step3.pack(fill=tk.X, pady=(0, 8))
        ttk.Checkbutton(step3, text="添加日期后缀", variable=self.add_date_suffix_var, command=self._update_rule_preview).pack(side=tk.LEFT)
        tk.Label(step3, text="MMDD:", bg="#FFFFFF").pack(side=tk.LEFT, padx=(6, 2))
        ttk.Entry(step3, textvariable=self.date_suffix_var, width=8).pack(side=tk.LEFT, padx=(0, 12))
        ttk.Checkbutton(step3, text="添加版本后缀", variable=self.add_version_suffix_var, command=self._update_rule_preview).pack(side=tk.LEFT)
        tk.Label(step3, text="版本:", bg="#FFFFFF").pack(side=tk.LEFT, padx=(6, 2))
        ttk.Combobox(step3, textvariable=self.version_suffix_var, width=6, state="readonly", values=[str(i) for i in range(1, 11)]).pack(side=tk.LEFT)

        step4 = ttk.LabelFrame(body, text="④ 执行操作", style="Card.TLabelframe", padding=10)
        step4.pack(fill=tk.X, pady=(0, 8))
        tk.Button(step4, text="扫描预览", command=self.scan_preview, bg="#2563EB", fg="#FFFFFF", relief=tk.FLAT, padx=12).pack(side=tk.LEFT)
        self.rename_btn = tk.Button(step4, text="开始重命名（0）", command=self.rename_all, state=tk.DISABLED, bg="#C9CDD3", fg="#7A7F87", relief=tk.FLAT, padx=12)
        self.rename_btn.pack(side=tk.LEFT, padx=8)
        tk.Button(step4, text="撤回上次重命名", command=self.undo_last_rename, bg="#F59E0B", fg="#FFFFFF", relief=tk.FLAT).pack(side=tk.LEFT, padx=8)
        ttk.Button(step4, text="导出预览CSV", command=self.export_preview_csv).pack(side=tk.LEFT, padx=8)
        tk.Button(step4, text="清空列表", command=self._clear_preview, bg="#6B7280", fg="#FFFFFF", relief=tk.FLAT).pack(side=tk.LEFT, padx=8)
        ...
        tk.Label(step4, text="冲突处理：", bg="#FFFFFF").pack(side=tk.LEFT, padx=(10, 2))
        self.conflict_cb = ttk.Combobox(step4, state="readonly", values=list(self.CONFLICT_LABEL_TO_KEY.keys()), width=12)
        self.conflict_cb.pack(side=tk.LEFT)
        self.conflict_cb.set(self.CONFLICT_LABELS["auto_index"])
        self.conflict_cb.bind("<<ComboboxSelected>>", self._on_conflict_change)

        tk.Label(body, textvariable=self.summary_var, bg="#F4F5F7", fg="#202124", font=("Microsoft YaHei UI", 10)).pack(fill=tk.X, pady=(0, 6))
    ...
    def scan_preview(self) -> None:
        ...
        self.summary_var.set(f"扫描结果：共 {len(self.preview_items)} 个项目（文件 {file_count} 个，文件夹 {folder_count} 个） | 可重命名：{ready_count} | 冲突跳过：{conflict_count} | 错误：{error_count}")
