...
class MainWindow(QMainWindow):
    ...
    def refresh_table(self):
        self.filtered_rows = self.filter_items() if self.items else []
        self.table.setRowCount(len(self.filtered_rows))
        for r, i in enumerate(self.filtered_rows):
            ...
            for c, v in enumerate(vals):
                ...
                if c == 0:
                    it.setData(Qt.UserRole, i.get("path", ""))
                self.table.setItem(r, c, it)

    def get_file_path_from_row(self, row: int) -> str:
        if row < 0 or row >= len(self.filtered_rows):
            return ""
        return str(self.filtered_rows[row].get("path", ""))
    ...
    def show_table_context_menu(self, position):
        ...
        if item is None:
            return
        row = item.row()
        file_path = self.get_file_path_from_row(row)
        if not file_path:
            return

        menu = QMenu(self)
        open_folder_action = menu.addAction("打开文件所在目录")
        reveal_file_action = menu.addAction("打开并选中文件")
        copy_file_path_action = menu.addAction("复制完整路径")
        copy_folder_path_action = menu.addAction("复制文件夹路径")
        action = menu.exec(self.table.viewport().mapToGlobal(position))

        if action == open_folder_action:
            self.open_file_folder(file_path)
        elif action == reveal_file_action:
    ...
    def open_file_folder(self, file_path: str):
        ...
        try:
            ...
            logging.exception("open folder failed for %s: %s", file_path, e)
            QMessageBox.warning(self, "提示", "打开目录失败，请检查系统环境")

    def reveal_file_in_explorer(self, file_path: str):
        path = Path(file_path)
        if not path.exists():
            QMessageBox.warning(self, "提示", "文件不存在，可能已被移动或删除")
            return
        try:
            ...
            logging.exception("reveal file failed for %s: %s", file_path, e)
            QMessageBox.warning(self, "提示", "打开并选中文件失败，请检查系统环境")
